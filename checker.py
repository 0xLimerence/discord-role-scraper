...
print(f'{Fore.RED}Discord Role Scraper v1.0 | Fantasy{Fore.RESET}\n')
...
try:
    ...
    for role in roles:
        role_info = []
        role_info.append(role['position'])
        role_info.append(emoji_pattern.sub(r'', role['name']).encode('ascii', 'ignore').decode('utf-8'))
        role_info.append(role['id'])
        role_info.append(f'{Fore.GREEN}Yes{Fore.RESET}' if int(role['permissions']) & 0x08 != 0 else f'{Fore.RED}No{Fore.RESET}') # Admin
        role_info.append(f'{Fore.GREEN}Yes{Fore.RESET}' if int(role['permissions']) & 0x010000000 != 0 else f'{Fore.RED}No{Fore.RESET}') # Manage roles
        role_info.append(f'{Fore.GREEN}Yes{Fore.RESET}' if int(role['permissions']) & 0x010 != 0 else f'{Fore.RED}No{Fore.RESET}') # Manage channels
        role_info.append(f'{Fore.GREEN}Yes{Fore.RESET}' if int(role['permissions']) & 0x020000 != 0 else f'{Fore.RED}No{Fore.RESET}') # @everyone
        role_info.append(f'{Fore.GREEN}Yes{Fore.RESET}' if int(role['permissions']) & 0x020000000 != 0 else f'{Fore.RED}No{Fore.RESET}') # Webhooks
        role_info.append('' + \
    ...
    tab_data = tabulate(data, headers=['Position', 'Role Name', 'Role ID', 'Admin?', 'Manage Roles?', 'Manage Channels?', '@Everyone?', 'Webhooks?', 'Tags'], tablefmt='github')
    print(f'\n{tab_data}' )
